<template>
  <div class="container mt-5">
    ...
    <div class="controls mb-3">
      ...
      <button @click="getUserLocation" class="btn btn-primary me-2">Показати моє місце</button>
    </div>
  ...
  methods: {
    ...
    updateMarkers() {
      ...
      filteredGyms.forEach(gym => {
        ...
        const appleMapsUrl = `https://maps.apple.com/?ll=${gym.lat},${gym.lng}&q=${encodeURIComponent(gym.name)}`;
    ...
    isOpenNow(hours) {
      ...
      const timeRange = isWeekend ? weekendHours.split(': ')[1] : weekdayHours.split(': ')[1];
    ...
    getNearestGym() {
      ...
      const nearest = this.gyms.reduce((closest, gym) => {
        ...
        navigator.geolocation.getCurrentPosition(
          position => {
            ...
            if (this.map) {
              ...
              this.userPositionMarker = L.marker([this.userPosition.lat, this.userPosition.lng], {
                icon: L.icon({
